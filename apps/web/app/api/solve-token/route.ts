...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // then use its OWN fix_runs id as the token's jti — jti and the reservation stay
  // correlated 1:1, so a leaked/abused token can be traced back to (and later
  // revoked via) the exact run it billed.
