...
export interface ICiWorkspaceReport {
  branch: string
  commit: string
  score: number | null
  grade: TGrade | null
...
export interface ICiWorkspaceRepo {
  slug: string
  ownerRepo: string
  /** The most recent report across all branches, or null when the repo is
   * registered but hasn't uploaded a report yet. */
  latestReport: ICiWorkspaceReport | null
}

/**
 * CI repos reachable from a workspace, each with its most recent report. Scope
 * is resolved through api_keys.workspace_id — ci_repos.user_id is the ACCOUNT
 * that originally registered the repo (immutable, never reassigned — see
 * upsertRepo), not the active workspace, so scoping on it would either leak a
 * repo registered under a different workspace of the same user, or hide one
 * registered by a teammate's key. This mirrors how the usage and API-keys
 * dashboard pages scope fix_runs through the same workspace -> api_keys link.
 */
export async function ciReposForWorkspace(workspaceId: string): Promise<ICiWorkspaceRepo[]> {
  if (!isSupabaseConfigured()) {
    return []
  }
  const client = await createServiceClient()
