...
export async function verifyApiKey(raw: string): Promise<IApiKey | null> {
  if (!isSupabaseConfigured()) {
    return devKey() !== undefined && raw === devKey()
      ? { id: DEV_KEY_ID, plan: EPlan.FREE, workspace_id: null }
      : null
  }
...
export async function findApiKeyById(id: IApiKey['id']): Promise<IApiKey | null> {
  if (!isSupabaseConfigured()) {
    return id === DEV_KEY_ID && devKey() !== undefined
      ? { id, plan: EPlan.FREE, workspace_id: null }
      : null
  }
  const client = await createServiceClient()
  const { data, error } = await client
 ...
 *
 * The bucket is scoped to the key's WORKSPACE (mirrors consume_metered_run):
 * every key minted under one workspace counts against the same total, so this
 * reported "used" always matches what actually gates the next reservation.
 * Legacy keys with no workspace_id keep their own independent per-key count.
 *
 * NOTE: the gate is still check-then-record at the call site, so tightly
 * concurrent requests near the limit can overshoot by a few. A DB-side atomic
 * counter is the proper fix (follow-up).
 */
async function meteredQuota(key: IApiKey): Promise<IFixQuota> {
  ...
  const { count, error } =
    key.workspace_id !== null
      ? await query.eq('workspace_id', key.workspace_id)
      : await query.eq('api_key_id', key.id)
 ...
 * counts the runs in the window and inserts the new run only if under the plan
 * limit, in a single advisory-locked transaction (the consume_metered_run DB
 * function). Replaces the racy check-then-record so concurrent requests can't
 * overshoot the quota. Returns the new run id, or null when the quota is
 * exhausted or on a metering error (fail closed). Callers refund via
 * {@link refundMeteredRun} when the downstream work then fails.
 *
 * Only the key id is sent — consume_metered_run resolves the CURRENT
 * workspace_id itself from api_keys (rather than trust a caller-supplied
 * value) and locks/counts on the workspace when the key belongs to one, so
 * every key in that workspace shares one bucket; legacy (workspace-less) keys
 * keep their own independent bucket.
 */
