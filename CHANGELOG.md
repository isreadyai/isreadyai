...
- Bumped the dompurify, postcss, nanoid and undici overrides past the audited advisories so CI's `bun audit` passes.
...
- Formatted the root README so the npm release job's oxfmt check passes.
