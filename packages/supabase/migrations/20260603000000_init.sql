...
begin
  ...
  insert into public.fix_runs (api_key_id, repo, url, patches, kind)
  values (p_api_key_id, p_repo, p_url, p_patches, p_kind)
...
CREATE TABLE public.fix_runs (
    ...
    workspace_id uuid,
    kind text,
    CONSTRAINT fix_runs_kind_check CHECK (((kind IS NULL) OR (kind = ANY (ARRAY['fix'::text, 'solve'::text]))))
);
